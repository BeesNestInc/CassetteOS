...
echo "⏹ Stopping wpa_supplicant@$INTERFACE..."
systemctl stop wpa_supplicant@$INTERFACE || true
[ -e /run/wpa_supplicant/$INTERFACE ] && rm -f /run/wpa_supplicant/$INTERFACE

echo "▶ Starting wpa_supplicant@$INTERFACE..."
systemctl start wpa_supplicant@$INTERFACE || exit 1
